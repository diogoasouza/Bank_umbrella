defmodule BankWeb.LoginControllerTest do
  ...
  test "renders the login page with an error flash when not all fields where filled", %{
    conn: conn
  } do
    conn =
      conn
  ...
  test "renders the login page with errors message when email is in the wrong format ", %{
    conn: conn
  } do
    conn =
      conn
  ...
  test "login and renders the summary page ", %{conn: conn} do
    ...
    destiny = Conn.get_resp_header(conn, "location")
    assert destiny == ["/"]
  end
  ...
  test "creates a new user and goes to the login page", %{conn: conn} do
    conn =
      ...
      |> post("/login/new", %{
        users: %{
          name: "Teste",
          email: "diogo.asouza" <> Integer.to_string(Enum.random(0..99_999_999)) <> "@gmail.com",
          password: "123"
        }
      })
  ...
  test "renders the create user page with an error flash when not all fields where filled", %{
    conn: conn
  } do
    conn =
      conn
  ...
  test "renders the create user page with an error when the email is already in use", %{
    conn: conn
  } do
    conn =
      conn
      |> post("/login/new", %{
        users: %{name: "Diogo", email: "diogo.asouza@gmail.com", password: "123"}
      })
