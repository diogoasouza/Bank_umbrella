defmodule Bank.AccountsQueries do
  ...
  def withdrawl(id, amount) when is_float(amount) do
    ...
    changes = Changeset.change(account, balance: account.balance - amount)
    Repo.update(changes)
  end
  ...
  def withdrawl(id, amount) do
    account = Repo.get!(Accounts, id)
    changes = Changeset.change(account, balance: account.balance - String.to_integer(amount))
    Repo.update(changes)
  end
